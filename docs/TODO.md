...
- [ ] 为 TypeScript Agent 增加串行消息队列。
  ...
  涉及文件：[agent.ts](/C:/work/cc-feishu/apps/typescript/src/claude/agent.ts)、[chat-manager.ts](/C:/work/cc-feishu/apps/typescript/src/bot/chat-manager.ts)

- [x] 恢复 TypeScript 版的 session 浏览与恢复能力。
  说明：TypeScript 版已补上 `/resume`、session 列表、编号恢复、跨目录恢复、空 session 过滤等流程，但还需要继续验证恢复语义和回归风险。
  涉及文件：[chat-manager.ts](/C:/work/cc-feishu/apps/typescript/src/bot/chat-manager.ts)、[message.handler.ts](/C:/work/cc-feishu/apps/typescript/src/handlers/message.handler.ts)

- [ ] 补齐 TypeScript 的 WebSocket 生命周期治理。
  说明：TypeScript 版现在的 `stop()` 还只是清空引用，不是真正关闭连接；需要补充连接关闭、异常退出上抛和对应回归验证。
  涉及文件：[websocket.ts](/C:/work/cc-feishu/apps/typescript/src/bot/websocket.ts)

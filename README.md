...
- `/cd <路径>` - 切换工作目录（`/cd .` 切换到根目录）
- `/resume` - 列出可用的 sessions
- `/resume <编号|session_id>` - 恢复到指定 session
...
1. **首次对话** - 在默认工作目录创建新会话
2. **后续对话** - 自动恢复之前的会话上下文
3. **切换项目** - 使用 `/cd <路径>` 切换到其他项目目录
4. **查看历史** - 使用 `/resume` 查看当前目录的所有 sessions
5. **恢复会话** - 使用 `/resume <编号>` 切换到历史会话
6. **重置会话** - 使用 `/new` 清空上下文，开始新对话
